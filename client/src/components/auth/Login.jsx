...
class Login extends React.Component {
  ...
  handleSubmit = async (event) => {
    ...
    await fetch(`/api/v1/user/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(user) })
      ...
      .then((data) => {
        ...
        this.props.updateUser(data.user, true, false)
      }).catch((err) => {
        localStorage.clear();
        this.state.errors.isLoggedError = true;
        this.setState({
          email: "",
          password: "",
  ...
  render () {
	...
	if (this.state.isLoggedIn || this.props.user) return <Redirect to="/" />;
    return (
			...
			<div className="  flex flex-row items-center justify-center py-20 h-96   shadow-2xl    ">
				...
				<div className="px-10">
					<form className="flex flex-col items-center justify-center">
						...
						<span>{this.state.errors.isLoggedError && "email and password doesn't match"}</span>
						...
						<div>
							<button onClick={this.handleSubmit} id="register" className="text-xl bg-red-700 text-white px-6 py-2 my-2">
								Login
							</button>
						</div>
						<span className=" text-shadow-xl font-semibold">
							Not Registered Yet ?{" "}
